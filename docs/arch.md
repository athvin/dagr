...
A developer writes units of work in Rust, declares how they connect, and compiles one binary. That binary *is* the pipeline — or, when the author declares several, **every** pipeline they wrote: each is auto-discovered and one is selected per invocation (`dagr run etl` versus `dagr run analytics`; ADR 092). Running it executes the selected graph in the right order, with the right concurrency, and leaves behind a record of what happened. By default it executes every node **in-process, on whatever machine it was launched on**; an opt-in executor can instead place individual nodes on remote compute for the duration of the run (ADR 115), which changes where a node runs and nothing else.

**Nothing the tool requires is a server, a scheduler, or a database**, and there is no configuration file describing the graph and no parsing step — the graph is expressed in code the compiler has already checked. Two of those words now have opt-in counterparts, and the honest shape of each is worth stating rather than implying: a **run index** (an embedded, SQLite-compatible libSQL *file* — off by default, coordinating nothing, ADR 097) may be written alongside the event stream, and the **remote executor** (ADR 115) depends on whatever cluster the operator points it at. Neither is required: the default path needs the binary, its arguments, and a directory. And under *both* executors dagr itself opens **no listener** — that half is unconditional, and machine-checked.

**Why there are two executors.** The pair exists to serve one workflow: **iterate locally at full speed until you are confident the pipeline is correct, then give each task the infrastructure its work actually needs.** Locally, every node runs in-process and values pass in memory — the fastest edit-run-inspect loop available, and the mode the testing surface (C28) is built around. In production, a node whose work wants 64 GiB, a GPU nodepool, or isolation from its neighbours gets exactly that, instead of forcing the whole run's admission pools (C12) to size for the hungriest node. The graph, the task bodies, and the recorded outcome are identical either way; only placement changes.
...
"Distributed execution system" here means an engine that distributes **the graph and its control** — cooperating orchestrators, work-stealing between them, cross-run queues, a control plane other processes hand off to and that outlives a run. What is **permitted** is a **single orchestrator process placing individual node attempts on remote compute it owns for the duration of one run**: one process still owns the graph, still owns the event stream, and still exits when the run ends. It gains no peer, no election, no shared lock, no queue, and no service; remote task pods report nothing inbound and never touch the run index. Remote execution changes *where* a node runs, never *how many* nodes there are and never *when* a pipeline runs. That carve-out is decided in ADR 115 (T100); like ADR 097's, it moves nothing else on this list.
...
**The run store.** One operator-supplied base location — a local path by default, supplied by flag or environment variable (`--store`, with `DAGR_STORE` as its fallback under the standard precedence) — under which everything a run leaves behind lives: `<base>/<pipeline>/<run-id>/` holds the event stream, both artifacts, and scratch. The event stream is written through an injected sink (append a line, flush); the default sink is a local file under the run store. Pointing the base at storage that survives the container — a mounted volume, a synced directory — is the operator's one job, and it is the *only* infrastructure this tool ever asks for. Runs whose store did not survive are not resumable; everything else works regardless.
...
**Behavior.** Attached at registration, never inside the task. Covers: retry count and backoff shape, per-attempt timeout, declared resource cost, trigger rule, execution class override, group membership, whether the output is retained after its consumers finish (C10), whether the output is durable (C27), and — since ADR 115 §7 — an optional **placement** carrying opaque strings only (CPU, memory, node selectors, tolerations), absent by default. Placement is policy rather than an execution class precisely so it feeds the *policy hash* and not the structural fingerprint (C21): moving a node between local and placed is then a printed policy diff a resume proceeds through, never a refusal, and a pipeline run locally can be resumed placed. Under the local executor (C26) a placement is recorded and ignored.
...
Defaults are conservative and stated explicitly: no retries, no timeout, zero declared cost, `all-succeeded`, execution class as declared by the task (await-bound if unspecified), no group, release the output once consumed, not durable, no placement (the node runs in this process).
...
**Purpose.** Stops the pipeline from exceeding the machine it is running on. This is a claim about **local, in-process execution**, which is what the admission pools govern; a node placed on remote compute (ADR 115) consumes almost none of this machine's capacity and is bounded by its own pool instead.

**Behavior.** Holds weighted capacity pools for the genuinely constrained resources — memory and threads at minimum, plus a **remote-slot** pool bounding concurrently-placed node attempts (ADR 115 §7). The remote pool is sized by a flat operator ceiling (`--dagr.max-pods`, C26) rather than derived from this machine, and is unconstrained until one is stated: it exists to cap in-flight remote work, not to mirror a cluster quota dagr cannot see. A placed node draws one remote slot and near-zero local capacity **when the executor honours its placement**; under the local executor it runs here and is charged its declared local cost like any other node. A ready node is admitted only when its declared cost fits the remaining capacity of *every* pool it needs; acquisition is all-or-nothing across pools (no permit is held while waiting for another — that is how two nodes deadlock). Admission order is oldest-ready-first with bounded bypass: a small node may jump the queue only when admitting it cannot delay the oldest waiter, so a large node is never starved by a stream of small ones.
...
**Execute one node from supplied references (opt-in, ADR 115 §3 — the pod-side verb).** `exec-node --run --node --attempt` plus the node's ordered input references runs **exactly one attempt** of that node in this process, from references given on the command line rather than read out of a prior run. It is the machine-facing sibling of *run a single node*, not a replacement for it: that verb is operator-facing and reads the run store, this one is what an executor invokes inside the compute it rented, and neither changes the other's behaviour. Because the remote side re-enters *the same binary*, the graph, the task code, the resource registry, and the codec are identical on both sides by construction, so nothing about the pipeline is transported and orchestrator-to-task version skew is impossible by default. The verb applies **no retry, backoff, or timeout** — those are the orchestrator's, and two retry loops would duplicate an attempt — and reports through an **attempt shard** rather than an artifact: one attempt's slice of the event stream (C19) in the same JSON Lines shape, plus the terminal state, the durable output reference and its metadata, the input references the attempt was actually given, and diagnostic strings. The shard is written to the blob store at an address derived from `(run, node, attempt)`, atomically and after the output blob, so a partial shard is never mistaken for a complete one and a shard's existence implies its output exists; it carries both fingerprints and the tool version so a reader can refuse one from a different build. A resource registry rebuilt this way exists **once per invocation** rather than once per run — a documented consequence, not a defect. Exit codes are the same table, with no new numbers. The verb is recognized by every binary; its body requires the blob backend, and a build without it refuses saying so.
...
**Many flows per binary (operator-approved M5 addition — ADR 086; auto-discovered since M6 — ADR 092).** A binary may host **many named flows** and select one per invocation: a small, additive `FlowRegistry` maps a flow **name → a re-invokable factory `Fn() -> RunnableFlow`**, and a `run_registry(&registry, argv)` entrypoint dispatches over it (`dagr run etl` versus `dagr run nightly`, plus `dagr list`). Selection is purely name-based over the existing single-run engine — each `dagr run <flow>` is its own independent run with its own run identity and store; it is *not* concurrent orchestration and *not* sub-DAG composition (both remain permanent non-goals). This adds one **optional positional to the command surface**: the parsed `Cli` gains a `flow_name: Option<String>` — the first positional token after the verb, or `None` when absent. The addition is backward-compatible: every existing verb, flag, and single-flow binary parses unchanged (a leading `--flag` is never a flow name), and a single-flow registry lets the name be omitted while a multi-flow registry requires it on `run`. Factories, not stored flows, are load-bearing because `RunnableFlow::run(self)` consumes the flow and it is not `Clone`, so one instance serves at most one verb. **M6 (ADR 092) made the registry implicit:** a `#[dag]` attribute over a `FlowBuilder` fn registers the flow through `inventory`, and `dagr_cli::run` auto-discovers every one in the binary — so adding a DAG needs no registry edit and `main` is one line. The hand-wired `FlowRegistry` remains a first-class fallback. Because `inventory` relies on life-before-`main` registration in a linker section, `#[dag]` declarations must live in the **leaf binary**: a `submit!` in an otherwise-unreferenced library is dropped by dead-code elimination, so cross-crate DAG libraries are unsupported.

**Environment fallbacks and the headroom knob (operator-approved M5 addition — ADR 089; wired into the run path by T114).** Every runtime knob honours the standard **`flag > env > default`** precedence **on the shipped run path**: the library `run <flow>` dispatch and the one-call `run_to_store` resolve the table below at bootstrap — parsing the reserved flags, folding in the environment, and validating — before the run store is opened, so a bad value leaves no run directory behind. The resolution helpers and the `RunConfig` builder methods remain opt-in library surface for a hand-wired caller, because `RunConfig::new()` stays infallible and environment-free by design; the standard entrypoints simply call them. So a knob can be set once in an orchestrator's environment and still be overridden per-invocation on the command line. One capacity nuance is deliberate: the admission pools stay **unconstrained** until a pool knob is supplied — a pin or an explicit headroom engages the container-limit probe (pins verbatim, un-pinned pools detected), and a run with no pool knob keeps the capacity behaviour it always had. The env namespace is `DAGR_*`, snake-case (flags stay kebab-case), documented alongside the startup-banner toggle `DAGR_NO_BANNER`:

| Flag | Env var | Default | Type | Validation |
|---|---|---|---|---|
| `--store` | `DAGR_STORE` | `./dagr-runs` | path | the run-store base — the one piece of infrastructure the spec asks an operator to supply, and therefore the knob that most needs to be settable once in an environment |
| `--grace` | `DAGR_GRACE` | 10s | Duration | `10` / `10s` / `10ms`; ≥ 1 ms |
| `--teardown-deadline` | `DAGR_TEARDOWN_DEADLINE` | 15s | Duration | `10` / `10s` / `10ms`; ≥ 1 s |
...
| `--dagr.executor` | `DAGR_EXECUTOR` | local | enum | `local` \| `k8s` — which executor runs this invocation's node attempts. Under `local`, a node's declared placement (C5) is **recorded and ignored** and everything runs in-process, so one binary is genuinely both. An unrecognized value fails loudly; a recognized-but-unimplemented one is refused at bootstrap rather than silently run locally |
| `--dagr.max-pods` | `DAGR_MAX_PODS` | unlimited | u32 | the flat operator ceiling on concurrently-placed node attempts — the `RemoteSlots` admission pool's capacity (C12). Unpinned by default: the cluster owns its own capacity and dagr does not second-guess it. `0` means no remote capacity, and a placed node then fails the bootstrap over-demand check |
| `--no-banner` | `DAGR_NO_BANNER` (or `NO_COLOR`) | banner shown | flag | present ⇒ suppress |
| `--dagr.log-format` | `DAGR_LOG_FORMAT` | structured | enum | `human` \| `structured` — the C25 output-mode selector. An unrecognized value fails loudly (`InvalidUsage`) listing the accepted set; unset or empty stays `structured` |

The headroom fraction (the admission-pool slack, C12) is exposed for the first time; the existing at-least-one-unit floor is unchanged, so even a `1.0` headroom still yields one unit per pool. A **bad env value fails loudly** and is never silently ignored or clamped: an unparseable value exits `InvalidUsage`, an out-of-range value (e.g. a headroom of `1.5`) exits `BootstrapFailure`, each diagnostic naming the offending source — the flag by its `--flag` spelling, the environment variable by its name, or (since ADR 128's `file` tier) the file, profile, and key an operator must edit. **`dagr-core` reads no environment** — the CLI resolves `DAGR_*` and passes already-parsed values inward (the `RunConfig` env-fallback builder methods and the pool pins), preserving the core's "reads the host once, injectable for tests" property (C12). This is a config *surface* only: it changes how a single invocation is configured, never *when* a pipeline runs. **There is no DSL, and no configuration file describing the graph** — both permanent scope boundaries. What *is* permitted, decided in ADR 128 (T113), is a **file of runtime knobs with named profiles** (`dagr.toml`, `[dev]` / `[prod]`), read at **bootstrap** and resolved as a fourth tier beneath the environment: `flag > env > file(profile) > default`. It configures **how one invocation runs** and nothing else: it cannot select which flow runs, declare nodes or edges, or reach node policy or placement, so the graph stays code the compiler has already checked. It is never read during assembly, which keeps assembly pure and C20's "empty environment with no configuration present" criterion intact, and `dagr-core` never reads it.
...
**Remote execution (opt-in, ADR 115).** An operator may select an executor that places individual node attempts on remote compute — today, one Kubernetes pod per node attempt, with per-node resource requests, selectors, and tolerations declared as node policy. The shape of the run does not change: **one orchestrator process** still owns the graph, admits nodes, writes the single event stream, and exits when the run ends. The remote pods re-enter the same binary, report nothing inbound, hold no credentials for the run index, and are observed through the cluster's own API by a single watch the orchestrator owns. When the orchestrator dies, outstanding pods are adopted on restart by rewriting an ownership label, never re-submitted. Because dagr owns retry, the cluster must not: Kubernetes-level retry and dagr-level retry running together would duplicate an attempt, so the executor requires it disabled. Kubernetes remains a legitimate *trigger* and is now also a legitimate *target*; the tool still never decides **when** a pipeline runs.
...
Designed for graphs of **ten to one thousand nodes**. Below ten, plain code is genuinely simpler (see "When not to use this"); far above a thousand, artifact sizes and diagram legibility degrade before the runtime does. Framework overhead per node — scheduling, admission, event writing; everything but the task's own work — is budgeted at **under one millisecond** **for local, in-process execution**, held by a CI benchmark that runs a thousand-node no-op graph and fails on regression. Artifact and stream sizes stay proportional to attempt count; the fixture corpus includes a ten-thousand-attempt artifact to keep the tooling honest at scale.

That sub-millisecond budget is a claim about **dagr's own work**, and it is scoped to local execution deliberately (ADR 115). Under a remote executor, the dominant cost is the cluster's: pod admission, image pull, and container start are measured in **seconds**, and no framework can hide that. Remote submission-to-start latency is therefore reported as its own figure, measured separately, and is explicitly **not** held to the per-node overhead budget — folding it in would make the benchmark's premise false rather than make the tool faster. The practical consequence for authors: a graph of many sub-second nodes should not be run remotely node-by-node; remote placement pays for itself when a node's own work dominates its startup cost.
...
7. **[machine]** Nothing in the tool requires a server, database, or scheduler to be running — the binary and its arguments are sufficient to run and to produce local artifacts. Crash-surviving artifacts and resume additionally require the one thing the operational model asks for: a run-store location on storage that outlives the container. This holds for the **default** executor, which is what "the tool requires" means: the opt-in remote executor (ADR 115) necessarily requires the cluster an operator asked it to use, and that is a chosen dependency, not one the tool imposes. Even then, no *dagr* server exists: the orchestrator opens no listener, remote task pods report nothing inbound, and nothing hands off to a service — so the "no server" half of this criterion is unconditional and is asserted structurally, not just for the default path.
...
**Post-M4 boundary amendments** — capability decisions that moved a *permanent* non-goal or scope boundary, each recorded in its own ADR and listed here so the spec's history is readable in one place:
- **ADR 097 (T82), 2026-07-27 — the run index.** "Metadata store" narrowed to mean a store the engine *depends on to coordinate*; a local, embedded, opt-in, non-coordinating index derived from the event stream permitted as an extension of the run store. Every other exclusion kept verbatim. ADR 012 superseded in part (its cross-run-index clause only).
- **ADR 115 (T100), 2026-07-29 — remote execution.** "Distributed execution system" narrowed to mean an engine that distributes *the graph and its control* (cooperating orchestrators, work-stealing, cross-run queues, a control plane outliving a run); a single orchestrator process placing individual node attempts on remote compute it owns for one run permitted. Remote pods report nothing inbound and never touch the run index; the event stream stays single-writer and remains the source of truth. Placement is node *policy*, not execution class, so it never refuses resume. The per-node overhead budget is scoped to local execution and remote start latency reported separately. ADR 012 superseded in part (its distributed-store clause only); ADR 014 superseded in part (its no-built-in-object-backend clause only). Every other exclusion kept verbatim.
- **ADR 128 (T113), 2026-07-29 — the runtime-knob configuration file.** C26's unqualified "there is no config file or DSL (a permanent scope boundary)" narrowed for the *config-file* half only: a file of runtime knobs with named profiles (`dagr.toml`), read at bootstrap and never during assembly, is permitted as a fourth precedence tier — `flag > env > file(profile) > default` — configuring how one invocation runs and nothing else. The file cannot select which flow runs, declare nodes or edges, or reach node policy or placement; "no DSL" and "no configuration file describing the graph" stay verbatim everywhere they appear, `dagr-core` never reads the file, and C20's "empty environment with no configuration present" criterion stands. The nine shipped `DAGR_*` spellings are unchanged, each mapped to one file key through a table CI proves total and injective. ADR 089 superseded in part (its config-file rejected-alternative half only); ADR 091 superseded in part (its restatement, same half). Every other exclusion kept verbatim.
