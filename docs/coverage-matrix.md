...
| C7 | machine | — | unmapped | T14 | Byte-identical graph artifacts; duplicate-name failure; empty-environment assembly; bootstrap rejects unsatisfiable cost/missing resource/invalid param (T13/T14/T15). **T13 (023) shipped the flow builder and node identity** (`dagr_core::flow`: `Flow`/`Pipeline`/`PipelineNode`), with runtime tests (`crates/core/tests/flow_builder.rs`) and compile-fail fixtures (`tests/ui/flow_pipeline_immutable`, `tests/ui/flow_finish_consumes_builder`). But **none of C7's machine acceptance criteria are covered by T13**: this criterion's owned tests — byte-identical graph artifacts, duplicate-name failure *naming both declarations*, empty-environment assembly, all-problems-not-first, exact consumer counts, no-parameter-reachable, and bootstrap rejections — are every one *explicitly deferred by T13's own Out of scope to T14/T15* (T13 lands only the builder skeleton, node identity, and the immutable pipeline; it lays the data seams and performs no assembly check). Re-pointed `Covered-by` T13→**T14** (the assembly-validation ticket that ships the duplicate-name/empty-environment/byte-identical tests) rather than mapping a T13 test that would not actually cover the criterion — a recorded re-point, not faked coverage. The rename-changes-identity / reorder-changes-nothing *node-identity* facet listed under C2 is likewise owned by **T12**'s consolidated wiring compile-fail suite (see the C2 row), so T13's identity tests do not re-point C2 either. |
