...
| SL7 | machine | — | System-level 7 | Nothing the **default** executor requires is a server, database, or scheduler running; the binary and its arguments suffice to run and produce local artifacts. The opt-in run index (ADR 097) and remote executor (ADR 115) are chosen dependencies, not imposed ones, and dagr opens no listener under either (crash-surviving artifacts and resume additionally need the operator-supplied durable run store) (T65). |
