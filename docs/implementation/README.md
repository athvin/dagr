...
## M9 — Rust best-practice hardening

Applies the `.claude/skills/rust-skills` guidance (265 rules / 26 categories, written against edition 2024) across the finished workspace. An audit found dagr already meets most of that bar — `clippy::all` + `pedantic` are denied workspace-wide, zero crate-level `#![allow]` escapes, 11 `as` casts in all of production, zero files missing `//!` docs, determinism already carried by a near-total `BTreeMap` preference. So the milestone is **not** a mechanical rule sweep, which would churn good code. It is three things: close the genuine configuration gaps (no `[profile.*]` exists anywhere; crate metadata would be rejected by crates.io; the feature matrix is untested in CI), take the **edition 2024 / MSRV** bump the guidance assumes, and leave behind a **machine-checked register** dispositioning every rule — so "why doesn't dagr use `thiserror`/`criterion`/`proptest`?" is answered once, in a file CI verifies, rather than re-litigated. Nothing here changes engine behaviour: `dagr-core` stays runtime-dependency-free, `panic = "abort"` stays forbidden (`execution::check_panic_strategy` refuses it), and the determinism jobs must stay byte-identical throughout. A lint promotion always ships in the same PR as the fixes that make it green. Work in order: the register (T92), profiles (T93), the edition bump (T94), error-chain and panic hardening (T95), docs/metadata/lint ratchet (T96), async and allocation review (T97), CI and test hygiene (T98), acceptance gate (T99). Note: ticket codes continue at **T92** because T82–T91 were used in M7/M8.
...
Lets an operator place **individual node attempts on their own Kubernetes pod**, with per-node CPU/memory/selectors/tolerations, while the default in-process executor stays byte-identical and stays the development path. Like M7, M10 opens with a decision **ticket** (**T100**), not a pre-accepted ADR record: it amends arch.md's *permanent* "distributed execution system" boundary — narrowing it to mean an engine that distributes *the graph and its control*, and permitting one orchestrator process that rents compute for one run — so it needs operator sign-off before merge (a STOP per ticket-conventions §8/§10), and it supersedes one clause each of ADR 012 and ADR 014.

The milestone is small because three shipped seams carry it. `NodeRunner` (`crates/cli/src/driver.rs:630`) is already the type-erased "where does this node run" boundary, so the executor is a new *implementation* and the run loop, readiness tracker, admission controller, teardown phase, and exit-code precedence do not change. `DurableOutput` is already a reference-carrying contract designed so a value can be rehydrated without re-running its producer. And because the metastore is a deterministic, idempotent projection of the event stream, **pods ship event-stream shards instead of writing to a database** — so nothing is served, no listener is added, pods hold no credentials, and the event stream keeps exactly one writer. `scripts/check-metastore-acceptance-boundary.sh` therefore passes unchanged, and M10 *extends* it rather than relaxing it.

Two invariants are load-bearing and are decided in T100, not re-litigated per ticket: **placement is node policy, never an `ExecutionClass` variant** (execution class feeds the *structural* fingerprint, so a `Remote` variant would refuse resume on every existing pipeline; policy divergence merely prints a diff), and **Kubernetes must not retry** (dagr already owns retry; both loops active duplicates an attempt). Two defects in shipped code are correctness prerequisites and land early as **T102**: the `RunnableFlow` adapter passes an empty timer to the retry loop, so backoff is emitted but never slept, and it arms no per-attempt timeout, so `NodePolicy::timeout` is fingerprinted but unenforced — a hung pod with no timeout would hang the run. **T101 is a spike and gates every ticket after it**: if watch reconnection, pod startup latency, or the shard path fail there, the decision reopens in T100 rather than being worked around locally.

One more invariant, added after review and decided in T100 (**ADR 115 §9**): an attempt's **submission** is recorded **write-ahead**, as a new `attempt-submitted` event kind (`dagr.event-stream@1.3`), before the remote work is created. Without it the platform's own work object is the only record of what an attempt was launched with — and that record is deleted on cleanup, so "what did this task read?" becomes unanswerable and a crash between deciding and submitting leaves nothing to recover from. It is an **event**, not a direct index write, because ADR 097 guarantees the index is a projection of the event stream and nothing else (CI asserts live-tee rows equal reconcile-produced rows); emitting an event gets the queryable row for free through the existing projection *and* adds append-only crash durability. A local run emits none of these records, so its stream stays byte-identical.

Work in order: the ADR (T100), the spike (T101), the retry/timeout prerequisites (T102), the payload codec (T103), the blob port (T104), placement and executor selection (T105), the pod-side verb (T106), the shared watch (T107), the executor and the write-ahead submission record (T108), orphan adoption (T109), the S3 backend and blob GC (T110), the submission projection and audit queries (T111), and the end-to-end gate (T112).

**T101 has run** (2026-07-29) against a real 4-node k3s cluster; its `## Spike findings` record the verdicts. All three bets hold — a long-lived watch is workable with a specific reconnect recipe, and pod startup is ~2.3s warm and flat to 10 concurrent pods — so **no ADR 115 decision reopens**. Two tickets took corrections from it: **T108**'s pre-start-failure detection (an unpullable image never reaches a terminal phase, so awaiting one hangs forever) and **T104**'s assumption that an RWX volume is available (the reference cluster has none). ⛔ **T112 is BLOCKED on operator-provisioned infrastructure** — a container registry, pod-to-pod storage (S3 or RWX), and a cluster with concurrency headroom. It carries a prerequisite block saying so; the loop must **halt and report** on reaching it rather than improvising a substitute.

- [ ] **115** · [T100 — ADR: remote-execution scope carve-out and Kubernetes substrate](115-T100-remote-execution-scope-and-substrate-adr.md) · S · decision
- [ ] **116** · [T101 — spike: kube watch semantics, pod startup latency, blob round-trip](116-T101-remote-execution-spike.md) · M · decision (spike) — after T100
- [ ] **117** · [T102 — real retry backoff and per-attempt timeout on the run-flow path](117-T102-backoff-and-timeout-enforcement.md) · M · feature — after T100
...
- [ ] **122** · [T107 — the shared pod observer: one watch, reconnect, identity](122-T107-shared-pod-observer.md) · L · feature — after T105
...
Gives an operator **one named place** to say how a run should be configured — `dagr.toml` with `[dev]` / `[prod]` profiles, layered under the existing flags and environment variables (`flag > env > file(profile) > default`). This is the surface M10's two executors were missing: "run it the way production runs it" stops being a dozen flags retyped by hand. The naming model is dbt's `profiles.yml`; the override model is `airflow.cfg`'s.

Like M7 and M10, M11 opens with a decision **ticket** (**T113**) because it moves a clause that calls itself permanent — **operator acceptance recorded 2026-07-29**, so it ships without halting. The audit behind it found that boundary is **thinner than the repo's boilerplate implies**: the *spec* forbids only a config file "**describing the graph**" (and the permanent non-goals list never mentions a config file at all), while the unqualified prohibition lives in just two sentences descending from ADR 089 — one of which **misquotes its own authority**, attributing to arch.md a claim about a "config surface" that arch.md makes in the opposite polarity. So the carve-out is narrow and clean: a file may say **how one invocation runs**; it may never say **what the graph is**. Flow selection, node/edge declaration, and policy or placement overrides stay permanently excluded, and the gate (T118) makes a `flow` key a rejected unknown rather than a convenience.

Two constraints are decided in T113 and enforced throughout. The file is read at **bootstrap, never during assembly** — `crates/core/tests/determinism_and_purity.rs` runs assembly with a cleared environment in an empty temp directory, and C20 requires artifact emission "in an empty environment with no configuration present", so a file consulted during assembly would break both *and* make the graph depend on ambient state. And **`dagr-core` never reads the file**, exactly as it never reads the environment; the TOML parser lives in `dagr-cli` alone.

The milestone opens with a prerequisite rather than the loader, because the audit found something load-bearing: **the environment tier ADR 089 specified is inert on the shipped path.** `registry.rs` builds `RunConfig::new(base).run_id(run_id)` and there are **zero** non-test callers of `grace_from_env`, `teardown_deadline_from_env`, `failure_mode_from_env`, `resolve_pool_pins`, or `resolve_headroom` — so `DAGR_GRACE=30s` currently changes nothing. A file tier beneath a tier nothing consults would resolve nothing, so **T114 wires resolution into the run path first** (and adds the documented-but-missing `DAGR_STORE`, fixes a `flag_takes_value` omission that makes `--dagr.metastore-store` swallow the flow name, and settles ADR 089's unimplemented duration bounds). That is also what finally makes arch.md's "every runtime knob honours the standard precedence" true.
...
Total: 80 M0–M4 tickets (all merged), plus 7 M5 tickets (**T71–T77**, docs 083–091) and 4 M6 tickets (**T78–T81**, docs 093–096), plus 7 M7 tickets (**T82–T88**, docs 097–103), 3 M8 tickets (**T89–T91**, docs 104–106), 8 M9 tickets (**T92–T99**, docs 107–114), 13 M10 tickets (**T100–T112**, docs 115–127), and 6 M11 tickets (**T113–T118**, docs 128–133). The M5 ADRs (082, 086, 089) and the M6 ADR (092) are decision records like ADR 081 and are not counted as tickets; M7's ADR (097 · **T82**) is instead a decision **ticket** — it amends arch.md's permanent boundary and needs operator sign-off, so it ships through the normal branch/PR flow. **M10's opening ticket (115 · T100) is the same shape and for the same reason**: it moves the permanent "distributed execution system" boundary and supersedes one clause each of ADR 012 and ADR 014. **M11's opening ticket (128 · T113) likewise** amends the config-file clause and supersedes one clause each of ADR 089 and ADR 091. Both carry recorded operator acceptance dated 2026-07-29, so neither halts. M9's opening ticket (107 · **T92**) is likewise a decision ticket: it records the disposition of every rule and the reasoning behind each non-adoption. `T0.1` (the spec-amendment pass) is already done and has no ticket.
