...
//! binding a `Handle<Label>` into a task whose `type Input = Count` reds the build —
...
//! mismatch (the `E0271` this case pins), not incidental `StableName`-bound noise.
...
fn main() {
    ...
    // Bind a `Handle<Label>` where `type Input = Count` is required: the exact-type
    // `Deps<Inputs = Count>` bound is not satisfied by `Handle<Label>`.
    let _ = f.node::<WantsCount, _>("wants-count", WantsCount, label);
}
