//! The typed handle — a typed claim on a value that does not exist yet.
//!
//! A [`Handle<T>`] is the **only** way one node refers to another node's output.
//! It carries the referenced node's identity plus the *type* of the value that
//! node will eventually produce, and it is obtainable **only** by registering a
//! node under an explicit name — there is no public constructor, no
//! `From`/`Default`, and no lookup by name, index, or string key. That single
//! property is what makes a dependency cycle *structurally* inexpressible rather
//! than something a later validation pass catches: a registration can only name
//! handles that already exist, so a forward reference cannot be written (see *No
//! forgeable path, no lookup* below).
//!
...
//! without a deliberate `pub` change that review will catch. The flow builder
//! that hands handles back lives in another module; this module delivers the
...
//! not from registration order. Renaming a node therefore changes its identity
//! (and, downstream, the structural fingerprint), while reordering registrations
//! changes nothing. This module consumes that contract; it does not re-derive
//! identity from order.
//!
...
//!   the ownership model — lives in the binding module; this module only provides
//!   the handle the binding consumes.
//! - **The flow builder, registration ergonomics, duplicate-name checking, and
//!   the stable-name trait/derive** live in the flow module; this module consumes
...
//!   ordering edges, wrong-type/arity binding, …) lives in the checked-in UI
//!   fixtures; this module guarantees only the public surface those fixtures
//!   assert against.

use std::marker::PhantomData;

/// A node's identity token — an opaque, name-derived key.
///
...
/// uniqueness check) can key on it, and `Debug` so it can appear in
...
impl NodeId {
    ...
    /// no registry**; it only hashes the name the caller already holds. The flow
    /// builder supplies the name from the author's declaration and pairs the
    /// resulting id with the node.
    ///
    /// # Not an identity escape hatch
    ///
    /// This is deliberately **public** because hand-construction of a
    /// [`RunContext`](crate::context::RunContext) names the nodes a teardown
    /// context covers, and a teardown developer identifies covered nodes by their
    /// author-declared name. Exposing it does **not** weaken the handle contract:
    /// a `NodeId` is an opaque identity token with no route back to a name or to a
    /// [`Handle`], and this function returns no [`Handle`] and reaches into no
    /// runtime node registry (dagr has none). The unforgeable-[`Handle`]/no-lookup
    /// contract is about *handles* and about a *lookup service*, neither of which
    /// this touches.
    #[must_use]
    pub fn from_name(name: &str) -> Self {
        ...
        // hash function is the fingerprint module's concern, not this equality
        // token's.
    ...
    /// stable sort key. Assembly sorts upstream-id lists by this to deduplicate
    /// and count dependencies deterministically; it is not a public route back to
    /// a name or a handle (identity stays opaque).
    #[must_use]
    ...
    /// durable scratch store can derive a stable, filesystem-safe per-node
    /// namespace segment from it. Two distinct nodes yield distinct fingerprints,
    /// so their scratch namespaces cannot collide; the same name always yields the
    /// same fingerprint, so a node's scratch is reachable across its own attempts.
    /// It is **not** a public route back to a name or a handle — identity stays
    /// opaque; the value is only ever rendered as an opaque hex directory name that
    /// reveals nothing about the node's declared name.
    #[must_use]
...
/// A typed claim on a value that does not exist yet.
///
...
/// matches a bound `Handle<T>`'s `T` against the consuming task's declared input
/// type, and a mismatch is a compile error.
pub struct Handle<T> {
    /// The referenced node's identity (name-derived). Private: a handle is
...
impl<T> Handle<T> {
    ...
    /// node registration, which is exactly what the flow builder calls here. Its
    /// existence is why a handle is unforgeable — there is no public path to it —
    ...
    /// The identity of the node this handle refers to (name-derived).
    ///
...
pub(crate) mod test_support {
    ...
    /// Mint a handle for a node registered under `name`, as the builder will.
    /// Test-only.
...
mod tests {
    ...
    /// node ("handles can be copied and passed around freely").
    #[test]
    ...
    /// ("reordering registrations changes nothing").
    #[test]
    ...
    /// solely the name ("renaming a node changes its identity").
    #[test]
    ...
    /// downstream byte-identical fingerprint.
    #[test]
