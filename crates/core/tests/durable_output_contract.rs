...
fn an_impl_may_supply_durable_reference_metadata() {
    ...
    assert_eq!(meta.scheme(), Some("file"));
    assert_eq!(meta.size_bytes(), Some("the-produced-bytes".len() as u64));
    assert_eq!(meta.produced_at_offset_ns(), Some(4242));
    let hash = meta.content_hash().expect("a content hash is supplied");
    assert!(
...
fn durable_reference_meta_fields_are_each_optional() {
    ...
    assert_eq!(meta.content_hash(), Some("sha256:abc"));
    assert_eq!(meta.size_bytes(), None);
    assert_eq!(meta.scheme(), None);
    assert_eq!(meta.produced_at_offset_ns(), None);
