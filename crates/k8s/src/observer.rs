...
//!
//! - [`ObserverCore`] is a **deterministic state machine**. It takes an
...
//!   network, or a sleep.
//! - [`PodObserver`] is the **task** that wires that machine to a [`PodApi`] and
//!   a timer, owns the one watch, and demultiplexes to per-attempt waiters.
//!
//! # Why one watch
//!
...
async fn observe<A: PodApi>(
    ...
    let mut watch: Option<PodWatch> = None;
    ...
    // Teardown: the watch goes with the run, and its pump with it.
    drop(watch);
